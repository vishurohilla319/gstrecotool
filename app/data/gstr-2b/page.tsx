...
export default async function Gstr2BPage() {
  ...
  const records = await prisma.gstr2B.findMany({
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-emerald-600 mb-1">
            <span>Static ITC Statement</span>
            <span>•</span>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Auto-drafted ITC statement generated on the 14th of each month under Section 16(2)(aa)
          </p>
        </div>
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-100 border-b border-slate-200 font-bold text-slate-700">
              <tr>
                ...
                <th className="px-3 py-3 text-center">ITC Available</th>
              ...
              {records.length === 0 ? (
                <tr>
                  <td colSpan={12} className="px-3 py-8 text-center text-slate-400">
                    No GSTR-2B data imported. Import file or click &quot;Load Demo Dataset&quot;.
                  </td>
                </tr>
              ) : (
                records.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-3 py-2.5 text-center">
                      {r.itcAvailability === "Y" && r.itcEligible ? (
                    ...
                    <td className="px-3 py-2.5 text-center">
                      {r.rcm ? (
                        <span className="bg-purple-100 text-purple-700 font-bold px-1.5 py-0.5 rounded text-[10px]">
                          RCM
                        </span>
                      ) : (
